...
class MFRecommender(nn.Module):
    ...
    def forward(self, u_idxs, i_idxs):
        ...
        # Combine
        return (dot + b).squeeze(1)


class ImplicitPairs(Dataset):
    def __init__(self, ratings_df, uid2idx, iid2idx, min_rating=4.0, negative_per_positive=1):
        # Keep only positives and only id that exist in mappings
        df = ratings_df.copy()
        pos = df[df["rating"] >= float(min_rating)][["userId", "movieId"]]
        pos = pos[pos["userId"].isin(uid2idx) & pos["movieId"].isin(iid2idx)]
        self.pos_pairs = pos.to_numpy(dtype=int).tolist()

        # Store mappings and parameters for negative sampling
        ...
        # All available items for negative sampling
...
class CollabTrainer:
    def __init__(
        ...
        self.dataset = ImplicitPairs(
            ratings_df, uid2idx, iid2idx, min_rating, negative_per_positive, max_users, max_items
        )
